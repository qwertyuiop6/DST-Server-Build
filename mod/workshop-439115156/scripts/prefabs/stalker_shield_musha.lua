...
local function StartRepel(inst)
    ...
    local creatures = {}
    for i, v in ipairs(TheSim:FindEntities(x, y, z, REPEL_RADIUS, { "locomotor" }, { "player", "musha", "companion", "playerghost", "INLIMBO" })) do
...
local prefs = {}
for i = 1, 4 do
    local name = "stalker_shield_musha"..tostring(i)
    table.insert(prefs, name)
    table.insert(ret, MakeShield(name, i))
end
table.insert(ret, MakeShield("stalker_shield_musha", nil, prefs))
prefs = nil
