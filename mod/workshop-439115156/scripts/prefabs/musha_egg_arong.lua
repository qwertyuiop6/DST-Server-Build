...
local function Hatch(inst)
    ...
    local amallarong = SpawnPrefab("arong_baby")
    amallarong.Transform:SetPosition(inst.Transform:GetWorldPosition())
    amallarong.sg:GoToState("grow_up")
...
local inst = CreateEntity()
    ...
    inst.Light:SetColour(95/255, 95/255, 95/255)
...
local function defaultfn()
   ...
   -- inst.components.hatchable:SetHatchTime(TUNING.SMALLBIRD_HATCH_TIME)
    inst.components.hatchable:SetHatchTime(150)
...
local function crackedfn()
   ...
   MakeInventoryPhysics(inst)
    ...
    inst.entity:AddMiniMapEntity()
	inst.MiniMapEntity:SetIcon( "musha_egg_cracked_arong.tex" )
    --inst.Light:Enable(false)
	 	inst.MiniMapEntity:SetPriority(10)
...
return Prefab( "common/inventory/musha_egg_arong", defaultfn, assets, prefabs),
